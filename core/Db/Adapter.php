...
class Adapter
{
    ...
    private static function getAdapterClassName($adapterName)
    {
        ...
        if (!class_exists($className)) {
            throw new \Exception(sprintf("Adapter '%s' is not valid. Maybe check that your configuration files in config/*.ini.php are readable by the webserver.", $adapterName));
        }
        return $className;
    }
